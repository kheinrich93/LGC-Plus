...
from utils.metric_tracking import MetricTrackerManager
...
class ITrainingLoop:
    ...
    def train(self):
        ...
        for epoch in range(self.parameter.epochs):
            ...
            # Training loop
            # for sample_idx in (tqdm.trange(self.training_generator.__len__())):
            for sample_idx in (range(self.training_generator.__len__())):
                input, reference = self.training_generator.__getitem__(sample_idx)
                metric_values = self.train_on_batch(input, reference, epoch)
                self.metric_tracking.update(True, self.metric_names, metric_values)

            # Validation loop
            for sample_idx in range(self.validation_generator.__len__()):
                input, reference = self.validation_generator.__getitem__(sample_idx)
                metric_values = self.validate_on_batch(input, reference, epoch)
            ...
            # hyperparm patience p, the number of times to observe worsening validation set loss before giving up
            # i = 0
            # j = 0
            # p = 2
            # v = np.inf

            # if val_loss does note decrease for p -> safe weight and break
            # if j <= p-1:
            #     i = i + 1
            #     v_curr = validation_loss
            #     if v_curr < v:
            #         j = 0
            #         v = v_curr
            #     else:
            #         j = j + 1
            # else:
            #     weights_path = self.model_path + 'weights_{:d}_{:.3f}_opt.h5'.format(epoch + 1, validation_loss)
            #     self.model.save_weights(filepath=weights_path)
            #     j = 0
            #     break
